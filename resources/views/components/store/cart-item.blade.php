    ...
    <div class="card-body d-flex gap-3 align-items-center">
        ...
        <div class="flex-grow-1">
            <div class="fw-semibold">
                {{ $item->item_type === 'pack' ? $item->pack?->name : $item->product?->name }}
            </div>
            ...
            <div class="small">
                @if($item->line_discount > 0)
                    <span class="text-decoration-line-through text-secondary">
                        ${{ number_format((float) $item->regular_price, 0, ',', '.') }}
                    </span>
                @endif

                <span class="fw-semibold">
                    ${{ number_format((float) $item->final_unit_price, 0, ',', '.') }}
                </span>
            </div>
        </div>
        ...
        <form
            method="POST"
            action="{{ route('store.cart.update') }}"
        ...
        <form
            method="POST"
            action="{{ route('store.cart.remove') }}"
        >
            @csrf
