<form method="POST" action="{{ route('pos.barcode.add') }}" class="row g-2 mb-3">
    @csrf
    <div class="col-md-9">
...
<form method="GET" action="{{ route('pos.sale.create') }}" class="mb-3">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    input?.addEventListener('input', () => {
        ...
        timer = setTimeout(async () => {
            ...
            const response = await fetch(`{{ route('pos.search') }}?q=${encodeURIComponent(q)}`);
            const rows = await response.json();
            results.innerHTML = rows.map(row => `
                <form method="POST" action="{{ route('pos.cart.add') }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
