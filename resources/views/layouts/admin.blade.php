...
<head>
    ...
    <title>@yield('title', 'Admin') | {{ $setting->store_name }}</title>
    ...
    <div class="admin-shell">
        <aside class="admin-sidebar p-3">
            <div class="d-flex align-items-center gap-2 mb-4">
                <a href="{{ route('admin.dashboard') }}" class="admin-brand d-flex align-items-center gap-2 text-white text-decoration-none flex-grow-1">
                    <span class="bg-white text-dark rounded px-2 py-1 fw-bold">QE</span>
                    <span class="admin-brand-text">
                        <span class="d-block fw-semibold">{{ $setting->store_name }}</span>
            ...
            <nav class="admin-nav accordion" id="adminSidebarNav">
                @foreach ($sections as $section => $items)
                    ...
                    <div class="admin-nav-section">
                        <button class="admin-section-toggle {{ $isOpen ? '' : 'collapsed' }}" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $sectionId }}" aria-expanded="{{ $isOpen ? 'true' : 'false' }}">
                            <span class="admin-section-title">{{ $section }}</span>
                            <span class="admin-section-chevron">⌄</span>
                        </button>
                        <div id="{{ $sectionId }}" class="collapse {{ $isOpen ? 'show' : '' }}" data-bs-parent="#adminSidebarNav">
                            <div class="nav nav-pills flex-column gap-1 pb-2">
                                @foreach ($items as $item)
                                    @php($isActive = isset($item['active']) && request()->routeIs($item['active']))
                                    @if ($item['disabled'] ?? false)
                                        <span class="nav-link {{ $isActive ? 'active' : 'disabled' }}">
                                            @isset($item['icon'])<i class="bi {{ $item['icon'] }} me-2"></i>@endisset
                                            <span class="admin-link-text">{{ $item['label'] }}</span>
                                        </span>
                                    @else
        ...
        <div class="admin-content flex-grow-1 d-flex flex-column">
            <nav class="navbar navbar-expand bg-white border-bottom px-3">
                <div>
                    <div class="fw-semibold">@yield('page-title', 'Panel administrativo')</div>
                    <div class="small text-secondary">{{ now()->format('d/m/Y H:i') }}</div>
                </div>
                <div class="ms-auto d-flex align-items-center gap-3">
                    <span class="badge text-bg-light">{{ auth()->user()->role?->name }}</span>
